...
async fn integration_init_middleware() {
    ...
    let (_, mut p2m_service, _) = init_middleware("10.0.0.1".to_string(), None, M2mNop, false);
...
async fn integration_o2m_local_provenance() {
    ...
    #[cfg(feature = "trace2e_tracing")]
    crate::trace2e_tracing::init();
    let (_, mut p2m_service, mut o2m_service) = init_middleware(String::new(), None, M2mNop, false);
