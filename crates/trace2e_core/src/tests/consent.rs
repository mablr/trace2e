...
async fn integration_consent_notification_local_and_remote_io() {
    ...
    let fd2_1_1 = FileMapping::new(2, 4, "/tmp/test2.txt", "10.0.0.2".to_string());

    local_enroll!(p2m_1, fd1_1_1);
    local_enroll!(p2m_2, fd2_1_1);
    ...
    let t = Instant::now();
    read!(p2m_1, fd1_1_1);
    info!(target: "consent_test", duration = t.elapsed().as_micros(), "Local file read");

    // Write to remote stream - this should generate another consent notification
    let t = Instant::now();
    write!(p2m_1, stream1_2);
    info!(target: "consent_test", duration = t.elapsed().as_micros(), "Stream write");

    // Further IOs on remote
    let t = Instant::now();
    read!(p2m_2, stream2_1);
    info!(target: "consent_test", duration = t.elapsed().as_micros(), "Stream read");

    let t = Instant::now();
    write!(p2m_2, fd2_1_1);
    info!(target: "consent_test", duration = t.elapsed().as_micros(), "Remote file write");

    // Check the notifications we received
    let (notification_count, destinations) = notifications_task.await.unwrap();

    // We should have received 4 notifications
    assert!(
        notification_count == 4,
        "Expected 4 consent notifications, got {}",
        notification_count
    );
