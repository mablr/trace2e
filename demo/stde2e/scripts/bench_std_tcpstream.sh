...
get_timings_m2m() {
    ...
    local write_reserve_req_time=$(echo "$write_reserve_r_begin - $write_reserve_l_sent" | bc)
    local write_reserve_processing_time=$(echo "$write_reserve_r_done - $write_reserve_r_begin" | bc)
    local write_reserve_ack_time=$(echo "$write_reserve_l_recv - $write_reserve_r_done" | bc)

    local write_sync_prov_req_time=$(echo "$write_sync_prov_r_begin - $write_sync_prov_l_sent" | bc)
    local write_sync_prov_processing_time=$(echo "$write_sync_prov_r_done - $write_sync_prov_r_begin" | bc)
    local write_sync_prov_ack_time=$(echo "$write_sync_prov_l_recv - $write_sync_prov_r_done" | bc)
