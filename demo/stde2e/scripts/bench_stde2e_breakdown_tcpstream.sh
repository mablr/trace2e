...
get_timings() {
    ...
    local enroll_req_time=$((enroll_m_begin - enroll_p_sent))
    local enroll_processing_time=$((enroll_m_done - enroll_m_begin))
    local enroll_ack_time=$((enroll_p_recv - enroll_m_done))

    local io_request_req_time=$((io_request_m_begin - io_request_p_sent))
    local io_request_processing_time=$((io_request_m_done - io_request_m_begin))
    local io_request_ack_time=$((io_request_p_recv - io_request_m_done))

    local io_time=$((io_report_p_sent - io_request_p_recv))

    local io_report_req_time=$((io_report_m_begin - io_report_p_sent))
    local io_report_processing_time=$((io_report_m_done - io_report_m_begin))
    local io_report_ack_time=$((io_report_p_recv - io_report_m_done))
...
while [ $i -lt $NB_ITER ];do
    LOG=$(echo 'RUST_LOG="trace2e_middleware::p2m_service=info" /trace2e_middleware & middleware=$! && RUST_LOG=trace2e_client=info /tcp_server_e2e -s 20; kill "$middleware"' | docker exec -i stde2e_server bash 2>/dev/null | ansi2txt & echo 'RUST_LOG="trace2e_middleware::p2m_service=info" /trace2e_middleware & middleware=$! && sleep 0.1 && RUST_LOG=trace2e_client=info /tcp_client_e2e -s 10 -a stde2e_server:8888; kill "$middleware"' | docker exec -i stde2e_client bash  2>/dev/null | ansi2txt)
    LOG_M2M=$(echo 'RUST_LOG="trace2e_middleware::m2m_service=info,trace2e_middleware::traceability=info" /trace2e_middleware & middleware=$! && /tcp_server_e2e -s 20; kill "$middleware"' | docker exec -i stde2e_server bash 2>/dev/null | ansi2txt & echo 'RUST_LOG="trace2e_middleware::m2m_service=info,trace2e_middleware::traceability=info" /trace2e_middleware & middleware=$! && sleep 0.1 && /tcp_client_e2e -s 10 -a stde2e_server:8888; kill "$middleware"' | docker exec -i stde2e_client bash  2>/dev/null | ansi2txt)
    ...
    FD_TO_WRITE=$(echo "$LOG" | grep "P-> remote_enroll" | grep "8888])$" | awk -F', ' '{print $2}')

    LOG_READ=$(echo "$LOG" | grep -v "$FD_TO_WRITE")
    LOG_WRITE=$(echo "$LOG" | grep "$FD_TO_WRITE")
