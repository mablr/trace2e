    ...
    c.bench_function("compliance_set_policy", |b| {
        b.to_async(tokio::runtime::Runtime::new().unwrap())
            .iter(|| async {
                ...
                let _ = black_box(
                    compliance
                        .call(ComplianceRequest::SetPolicy {
                            resource: process,
                            policy,
                        })
                        .await,
        ...
        b.to_async(tokio::runtime::Runtime::new().unwrap())
            .iter(|| async {
                ...
                let source_policies = HashMap::from([
                    ...
                    (
                        "10.0.0.1".to_string(),
                        HashSet::from([high_policy, medium_policy]),
                    ),
    ...
    c.bench_function("sequencer_drop_flow", |b| {
        b.to_async(tokio::runtime::Runtime::new().unwrap())
            .iter(|| async {
                ...
                let _ = black_box(
                    sequencer
                        .call(SequencerRequest::ReleaseFlow { destination: file })
                        .await,
                );
            });
    });
    ...
    c.bench_function("provenance_get_prov_empty", |b| {
        b.to_async(tokio::runtime::Runtime::new().unwrap())
            .iter(|| async {
                ...
                let _ = black_box(
                    provenance
                        .call(ProvenanceRequest::GetReferences(process))
                        .await,
                );
            });
    });
